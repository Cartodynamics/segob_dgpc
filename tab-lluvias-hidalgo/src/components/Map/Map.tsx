...
const attachAllTooltipEvents = useCallback((map: MaplibreMap) => {
  ...
  const htmlRegion = (props: any) =>
    `<strong>Municipio:</strong> ${props.NOMGEO ?? '-'}<br/><strong>Region:</strong> ${props.Region ?? '-'}`;
...
const getVectorLayerId = async (pmtilesUrl: string): Promise<string> => {
  ...
  // 👇 Esta línea es la clave: castear el resultado a any
  const md = (await p.getMetadata()) as any;

  const id = md?.vector_layers?.[0]?.id;
  if (!id) {
    throw new Error(`No encontré "vector_layers" en ${httpUrl}. Asegúrate de que el PMTiles es vectorial (MVT).`);
  }
...
const addVectorLayers = async (map: maplibregl.Map) => {
  const regions = [
    { id: 'region1', url: 'pmtiles://data/region1.pmtiles', color: '#70613F' },
    { id: 'region2', url: 'pmtiles://data/region2.pmtiles', color: '#9D792A' },
    { id: 'region3', url: 'pmtiles://data/region3.pmtiles', color: '#DEB52D' },
    { id: 'region4', url: 'pmtiles://data/region4.pmtiles', color: '#BDAA76' },
  ];

  for (const r of regions) {
    if (!map.getSource(r.id)) {
      map.addSource(r.id, { type: 'vector', url: r.url });
    }
    if (!map.getLayer(r.id)) {
      const sourceLayer = await getVectorLayerId(r.url);
      map.addLayer({
        id: r.id,
        type: 'fill',
        source: r.id,
        ...
        paint: {
          'fill-color': r.color,
          'fill-opacity': 0.48,
          'fill-outline-color': '#534444'
        }
      });
    }
  }

  const inund = { id: 'inundaciones_conagua', url: 'pmtiles://data/inundaciones_conagua.pmtiles' };
  if (!map.getSource(inund.id)) {
    map.addSource(inund.id, { type: 'vector', url: inund.url });
  }
  if (!map.getLayer(inund.id)) {
    const sourceLayer = await getVectorLayerId(inund.url);
    map.addLayer({
      id: inund.id,
      type: 'circle',
      source: inund.id,
  ...
  container,
  style: baseStyleUrl,
  center: [-98.79, 20.49], // Centro aproximado de Hidalgo
  zoom: 7.3,               // Nivel de estado
  pitch: 0,
  bearing: 0,
